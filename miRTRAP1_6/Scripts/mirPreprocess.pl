...
my($hairpins) = miRTRAP1_6::readMirbaseGff3($gffFile);
my $chromSizesFile = $parameters->{SizesFile};
my $genomeDir = $parameters->{genomeDir};
my $chromLengths = miRTRAP1_6::readChromLengths($chromSizesFile);
my $bamList = miRTRAP1_6::loadBamList($bamListFile);
miRTRAP1_6::mirPreprocess($bamList,$hairpins,$genomeDir,$chromLengths,$parameters);
