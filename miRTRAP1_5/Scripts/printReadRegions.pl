...
unless ($parameters->{LoadFromConfigFile} eq "") {
    ...
    miRTRAP1_5::readConfigFile($configFile,$parameters);
}
...
my $chromLengths = miRTRAP1_5::readChromLengths($chromSizesFile) or die "Failed to load chrom.sizes file.\n";
my $bamList = miRTRAP1_5::loadBamList($bamListFile) or die "Failed to load bamList file.\n";
my $repeatRegions = {};
if($repeatRegionsFile) {
    $repeatRegions = miRTRAP1_5::loadRepeatRegions($repeatRegionsFile, $chromLengths) or die "Failed to load repeat regions file.\n";
}
miRTRAP1_5::printReadRegions($bamList, $chromLengths, $repeatRegions, $parameters);
