...
\t--minOverlap\tThe minimum amount of overlap for it to be considered unlikely that its a true mir
...
GetOptions ("countMinLocus=i" => \$parameters->{countMinLocus},
	    ...
	    "minOverlap=i" => \$parameters->{minOverlap},
...
sub evaluateReadRegions {
	...
	foreach my $hairpin (@{$hairpinList->{$chrom}}) {
		...
		unless (dontPrint($reason,$parameters)) {
		    ...
		    if ( $reason =~ /Fails\sbecause\stotal\sreads\s=\s.*\s<\s$minLocusCount\s=\sminLocusCount\./ ) {
...
sub evaluate {
	...
	if(hasSpacedProducts($tag,$totalSense,$hairpinProducts,$parameters)) {
...
sub readValidList {
	...
	unless(/\#/) {
	    ...
	    my($chrom,$start,$stop) = miRTRAP1_5::parseLocation($location);	    
